...
export const getOrderById = async (orderId: number) => {
	...
	const { data: order, error } = await supabase
		.from('orders')
		.select(
			'*, addresses(*), customers(full_name, email), orders_items(quantity, price, variants(color_name, storage, products(name, images)))'
		)
		.eq('customer_id', customerId)
		.eq('id', orderId)
		.single();
	...
	return {
		...
		orderItems: order.orders_items.map(item => ({
			quantity: item.quantity,
			price: item.price,
